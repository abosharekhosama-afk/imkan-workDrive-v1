...
export function FileDetailsModal({
  ...
  if (data.createdAt) { facts.push({ term: label("files.column.created"), value: formatDateLocalized(data.createdAt, locale) }); }
  if (data.updatedAt) {
    facts.push({ term: label("files.column.updated"), value: formatDateLocalized(data.updatedAt, locale) });
  }
  if (data.tags?.length) { facts.push({ term: label("files.tags"), value: data.tags.join(", ") }); }
