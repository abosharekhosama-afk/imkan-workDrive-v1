...
describe('IDOR live integration (cross-tenant, MySQL)', () => {
  ...
  beforeAll(async () => {
    ...
    await ensureSession(prisma, tenantA.admin);
    await ensureSession(prisma, tenantB.admin);
  ...
  async function cleanupTenant(orgId: string): Promise<void> {
    await prisma.fileActivity.deleteMany({ where: { orgId } });
    await prisma.fileTag.deleteMany({ where: { tag: { orgId } } });
    await prisma.tag.deleteMany({ where: { orgId } });
    await prisma.comment.deleteMany({ where: { orgId } });
    await prisma.favorite.deleteMany({ where: { orgId } });
    await prisma.notification.deleteMany({ where: { orgId } });
    await prisma.accessEvent.deleteMany({ where: { orgId } });
    await prisma.trashEntry.deleteMany({ where: { orgId } });
    await prisma.fileShareRecipient.deleteMany({ where: { orgId } });
    await prisma.fileShare.deleteMany({ where: { orgId } });
    ...
    await prisma.user.deleteMany({ where: { id: { in: (await prisma.organizationMembership.findMany({ where: { organizationId: orgId }, select: { userId: true } })).map(m => m.userId) } } });
