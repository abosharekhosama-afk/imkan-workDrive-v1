...
async function ensureSession(prismaClient: PrismaClient, user: TenantUser): Promise<void> {
  if (sessionTokens.has(user.id)) return;
...
describe('Team Folder ACL live integration (same-org non-member, MySQL)', () => {
  ...
  beforeAll(async () => {
    ...
    for (const user of [admin, member, viewer, editor, organizer, tfAdmin, invitee, soloAdmin, adminB]) {
      await ensureSession(prisma, user);
    }
  ...
  afterAll(async () => {
    async function cleanupOrg(targetOrgId: string) {
      await prisma.fileActivity.deleteMany({ where: { orgId: targetOrgId } });
      await prisma.fileTag.deleteMany({ where: { tag: { orgId: targetOrgId } } });
      await prisma.tag.deleteMany({ where: { orgId: targetOrgId } });
      await prisma.comment.deleteMany({ where: { orgId: targetOrgId } });
      await prisma.favorite.deleteMany({ where: { orgId: targetOrgId } });
      await prisma.notification.deleteMany({ where: { orgId: targetOrgId } });
      await prisma.accessEvent.deleteMany({ where: { orgId: targetOrgId } });
      await prisma.trashEntry.deleteMany({ where: { orgId: targetOrgId } });
      await prisma.fileShareRecipient.deleteMany({ where: { orgId: targetOrgId } });
      await prisma.fileShare.deleteMany({ where: { orgId: targetOrgId } });
      ...
      await prisma.storageObject.deleteMany({ where: { orgId: targetOrgId } });
      await prisma.session.deleteMany({ where: { orgId: targetOrgId } });
      await prisma.storageQuota.deleteMany({ where: { orgId: targetOrgId } });
