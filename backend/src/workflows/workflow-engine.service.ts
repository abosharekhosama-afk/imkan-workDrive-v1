...
type WorkflowDefinition = { trigger?: string | string[]; condition?: unknown; actions?: WorkflowAction[]; states?: unknown[]; transitions?: unknown[]; start?: Record<string, unknown> };
type WorkflowFieldValues = Record<string, unknown>;
...
export class WorkflowEngineService implements OnModuleInit, OnModuleDestroy {
  ...
  async onFileEvent(user: AccessTokenPayload, event: WorkflowFileEvent): Promise<void> {
    ...
    for (const workflow of workflows) {
      ...
      const aliases: Record<string, string[]> = {
        ...
        PROPERTIES_UPDATED: ['properties_updated', 'PROPERTIES_UPDATED'], properties_updated: ['PROPERTIES_UPDATED', 'properties_updated'],
        READY: ['ready', 'READY'], ready: ['READY', 'ready'],
        FOLDER_COPIED: ['copy', 'FOLDER_COPIED'], FILE_COPIED: ['copy', 'FILE_COPIED'], copy: ['FILE_COPIED', 'FOLDER_COPIED', 'copy'],
      };
  ...
  private definitionFromWorkflow(workflow: { steps: Array<{ kind: string; config: unknown }>; states?: unknown[]; transitions?: unknown[] }): WorkflowDefinition {
    ...
    const triggerObject = triggerValue && typeof triggerValue === 'object' && !Array.isArray(triggerValue) ? triggerValue as Record<string, unknown> : null;
    return { trigger: Array.isArray(triggerObject?.events) ? triggerObject.events.map(String) : (Array.isArray(triggerValue) ? triggerValue.map(String) : String(triggerValue ?? '')), start: triggerObject?.start && typeof triggerObject.start === 'object' ? triggerObject.start as Record<string, unknown> : undefined, condition: values.CONDITION ?? 'any', actions, states: workflow.states, transitions: workflow.transitions };
  }

  private conditionMatches(condition: unknown, event: WorkflowFileEvent, fieldValues: WorkflowFieldValues = {}): boolean {
    if (!condition || condition === 'any') return true;
    if (Array.isArray(condition)) return condition.every((c) => this.conditionMatches(c, event, fieldValues));
    ...
    if (c.all && Array.isArray(c.all) && !c.all.every((x) => this.conditionMatches(x, event, fieldValues))) return false;
    if (c.any && Array.isArray(c.any) && !c.any.some((x) => this.conditionMatches(x, event, fieldValues))) return false;
    if (typeof c.field === 'string') { const actual = fieldValues[c.field] ?? (event as unknown as Record<string, unknown>)[c.field]; const expected = c.value; const op = String(c.operator ?? 'equals'); if (op === 'equals' && String(actual ?? '') !== String(expected ?? '')) return false; if (op === 'not_equals' && String(actual ?? '') === String(expected ?? '')) return false; if (op === 'contains' && !String(actual ?? '').toLowerCase().includes(String(expected ?? '').toLowerCase())) return false; }
  ...
  private async executeJob(jobId: string) {
    ...
    const result = (job.run.result && typeof job.run.result === 'object' ? job.run.result : {}) as Record<string, unknown>;
    ...
    const fieldValues = (result.fieldValues && typeof result.fieldValues === 'object' ? result.fieldValues : {}) as WorkflowFieldValues;
    ...
    try {
      await this.prisma.workflowRun.update({ where: { id: job.run.id }, data: { status: 'RUNNING', error: null } });
      let currentStateId = typeof result.currentStateId === 'string' ? result.currentStateId : null;
      let allResults: unknown[] = Array.isArray(result.actions) ? result.actions : [];
      ...
      const runActions = async (actions: WorkflowAction[]) => {
        for (let i = 0; i < actions.length; i++) {
          ...
          try {
            const output = await this.executeAction(token, event, action, job.workflow.id, job.run.id, currentStateId, fieldValues);
            allResults.push(output);
            await this.prisma.workflowStepRun.update({ where: { id: step.id }, data: { status: 'SUCCEEDED', output: output as unknown as Prisma.InputJsonValue, finishedAt: new Date() } });
            if (Boolean((output as { waiting?: boolean } | null)?.waiting)) return true;
          } catch (error) {
      ...
      if (continuation) {
        const destination = job.workflow.states.find(s => s.id === continuation.toStateId);
        const transitionActions = (continuation.actions as unknown as WorkflowAction[]) ?? [];
        const before = transitionActions.filter(a => (a as WorkflowAction & { phase?: string }).phase === 'before');
        const during = transitionActions.filter(a => ((a as WorkflowAction & { phase?: string }).phase ?? 'during') === 'during');
        const after = transitionActions.filter(a => (a as WorkflowAction & { phase?: string }).phase === 'after');
        if (await runActions(before)) return this.finishWaiting(job, allResults, currentStateId, job.id);
        if (await runActions(during)) return this.finishWaiting(job, allResults, currentStateId, job.id);
        currentStateId = destination?.id ?? null;
        await this.prisma.workflowRun.update({ where: { id: job.run.id }, data: { currentStateId, result: { ...result, currentStateId, continueTransitionId: null, fieldValues, actions: allResults } as unknown as Prisma.InputJsonValue } });
        if (await runActions(after)) return this.finishWaiting(job, allResults, currentStateId, job.id);
      } else {
        const start = definition.start ?? {};
        const startState = job.workflow.states.find(s => s.id === job.workflow.states[Number(start.to ?? 0)]?.id) ?? job.workflow.states[Number(start.to ?? 0)];
        if (!startState) throw new Error('Workflow has no starting state');
        if (!this.conditionMatches(definition.condition, event, fieldValues) || !this.conditionMatches(start.condition, event, fieldValues)) {
          await this.prisma.workflowRun.update({ where: { id: job.run.id }, data: { status: 'SUCCEEDED', result: { ...result, skipped: true, reason: 'Condition did not match' } as unknown as Prisma.InputJsonValue, finishedAt: new Date() } });
          await this.prisma.workflowJob.update({ where: { id: job.id }, data: { status: 'SUCCEEDED', lockedAt: null, lockedBy: null } });
          return;
        }
        const startActions = definition.actions ?? [];
        const startDuring = startActions.filter(a => ((a as WorkflowAction & { phase?: string }).phase ?? 'during') === 'during');
        const startAfter = startActions.filter(a => (a as WorkflowAction & { phase?: string }).phase === 'after');
        if (await runActions(startDuring)) return this.finishWaiting(job, allResults, currentStateId, job.id);
        currentStateId = startState.id;
        await this.prisma.workflowRun.update({ where: { id: job.run.id }, data: { currentStateId, result: { ...result, currentStateId, fieldValues, actions: allResults } as unknown as Prisma.InputJsonValue } });
        if (await runActions(startAfter)) return this.finishWaiting(job, allResults, currentStateId, job.id);
      }

      for (let guard = 0; guard < 30; guard++) {
        const state = currentStateId ? job.workflow.states.find(s => s.id === currentStateId) : null;
        if (!state) throw new Error('Workflow state not found');
        if (state.terminal) {
          await this.prisma.workflowRun.update({ where: { id: job.run.id }, data: { status: 'SUCCEEDED', currentStateId: state.id, result: { ...result, currentStateId: state.id, fieldValues, actions: allResults } as unknown as Prisma.InputJsonValue, finishedAt: new Date() } });
          await this.prisma.workflowJob.update({ where: { id: job.id }, data: { status: 'SUCCEEDED', lockedAt: null, lockedBy: null } });
          return;
        }
        const outgoing = job.workflow.transitions.filter(t => t.fromStateId === state.id && this.conditionMatches(t.condition, event, fieldValues));
        const manual = outgoing.filter(t => t.trigger === 'manual');
        if (manual.length) {
          const existingTask = await this.prisma.workflowTask.findFirst({ where: { runId: job.run.id, stateId: state.id, status: 'PENDING' } });
          if (!existingTask) {
            await this.prisma.workflowTask.create({ data: { orgId: job.orgId, workflowId: job.workflow.id, runId: job.run.id, stateId: state.id, assigneeId: user.id, title: `Action required: ${state.name}` } });
          }
          await this.prisma.workflowRun.update({ where: { id: job.run.id }, data: { status: 'WAITING', currentStateId: state.id, result: { ...result, currentStateId: state.id, fieldValues, availableTransitionIds: manual.map(t => t.id), actions: allResults } as unknown as Prisma.InputJsonValue } });
          await this.prisma.workflowJob.update({ where: { id: job.id }, data: { status: 'SUCCEEDED', lockedAt: null, lockedBy: null } });
          return;
        }
        const automatic = outgoing.find(t => t.trigger !== 'manual');
        if (!automatic) {
          await this.prisma.workflowRun.update({ where: { id: job.run.id }, data: { status: 'WAITING', currentStateId: state.id, result: { ...result, currentStateId: state.id, fieldValues, actions: allResults, blocked: true } as unknown as Prisma.InputJsonValue } });
          await this.prisma.workflowJob.update({ where: { id: job.id }, data: { status: 'SUCCEEDED', lockedAt: null, lockedBy: null } });
          return;
        }
        const destination = job.workflow.states.find(s => s.id === automatic.toStateId);
        if (!destination) throw new Error('Transition destination state not found');
        const actions = (automatic.actions as unknown as WorkflowAction[]) ?? [];
        const before = actions.filter(a => (a as WorkflowAction & { phase?: string }).phase === 'before');
        const during = actions.filter(a => ((a as WorkflowAction & { phase?: string }).phase ?? 'during') === 'during');
        const after = actions.filter(a => (a as WorkflowAction & { phase?: string }).phase === 'after');
        if (await runActions(before)) return this.finishWaiting(job, allResults, currentStateId, job.id);
        if (await runActions(during)) return this.finishWaiting(job, allResults, currentStateId, job.id);
        currentStateId = destination.id;
        if (await runActions(after)) return this.finishWaiting(job, allResults, currentStateId, job.id);
        await this.prisma.workflowRun.update({ where: { id: job.run.id }, data: { currentStateId, result: { ...result, currentStateId, fieldValues, actions: allResults } as unknown as Prisma.InputJsonValue } });
      }
      throw new Error('Workflow exceeded the maximum transition depth');
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
  ...
  private async finishWaiting(job: any, results: unknown[], currentStateId: string | null, jobId: string) {
    await this.prisma.workflowRun.update({ where: { id: job.run.id }, data: { status: 'WAITING', currentStateId, result: { actions: results, currentStateId } as unknown as Prisma.InputJsonValue } });
  ...
  private interpolateConfig(value: Record<string, unknown>, event: WorkflowFileEvent, fields: WorkflowFieldValues): Record<string, unknown> {
    const replace = (input: unknown): unknown => {
      if (typeof input === 'string') return input.replace(/\{\{\s*([^}]+)\s*\}\}/g, (_, key: string) => { const path = key.trim(); if (path === 'file.name') return event.name; if (path === 'file.id') return event.fileId; if (path === 'file.extension') return String(event.extension ?? ''); return String(fields[path] ?? ''); });
  ...
  private async executeAction(user: AccessTokenPayload, event: WorkflowFileEvent, action: WorkflowAction, workflowId: string, runId: string, currentStateId: string | null = null, fieldValues: WorkflowFieldValues = {}) {
    const config = this.interpolateConfig(action.config ?? {}, event, fieldValues);
    switch (action.type) {
      case 'notify': {
        const recipientIds = Array.isArray(config.userIds) ? config.userIds.filter((id): id is string => typeof id === 'string') : (typeof config.userId === 'string' && config.userId ? [config.userId] : [user.sub]);
      ...
      case 'share': {
        ...
        const recipientUserIds = Array.isArray(config.userIds) ? config.userIds.filter((id): id is string => typeof id === 'string') : (typeof config.userId === 'string' && config.userId ? [config.userId] : []);
      ...
      case 'request_approval': {
        const state = currentStateId ? await this.prisma.workflowState.findFirst({ where: { id: currentStateId, workflowId } }) : await this.prisma.workflowState.findFirst({ where: { workflowId }, orderBy: { position: 'asc' } });
