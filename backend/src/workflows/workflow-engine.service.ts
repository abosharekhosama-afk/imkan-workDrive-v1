...
export class WorkflowEngineService implements OnModuleInit, OnModuleDestroy {
  ...
  async startManual(user: AccessTokenPayload, workflowId: string, event: WorkflowFileEvent) {
    ...
    for (const field of fields) {
      ...
      if (!id) continue;
      let value = fieldValues[id];
      const emptyBeforeDefault = value === undefined || value === null || (typeof value === 'string' && !value.trim());
      if (emptyBeforeDefault && field.defaultValue !== undefined) { fieldValues[id] = field.defaultValue; value = field.defaultValue; }
      const empty = value === undefined || value === null || (typeof value === 'string' && !value.trim());
      if (field.required === true && empty) errors.push(`Field "${String(field.name ?? id)}" is required`);
      if (!empty && field.max !== undefined && typeof value === 'string' && value.length > Number(field.max)) errors.push(`Field "${String(field.name ?? id)}" exceeds the maximum length`);
      if (!empty && field.type === 'email' && typeof value === 'string' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) errors.push(`Field "${String(field.name ?? id)}" must be a valid email address`);
      if (!empty && field.type === 'number' && (typeof value !== 'number' || !Number.isFinite(value))) errors.push(`Field "${String(field.name ?? id)}" must be a number`);
      if (!empty && field.type === 'boolean' && typeof value !== 'boolean') errors.push(`Field "${String(field.name ?? id)}" must be yes/no`);
      if (!empty && field.type === 'choice' && Array.isArray(field.options) && !field.options.map(String).includes(String(value))) errors.push(`Field "${String(field.name ?? id)}" has an invalid choice`);
    }
    if (errors.length) throw new BadRequestException(errors.join('; '));
    const startParticipantRules = Array.isArray(input.participantRules) ? input.participantRules : undefined;
    const starterParticipantAllowed = definition.transitions.some((t) => this.phaseActions(t.actions).before.concat(this.phaseActions(t.actions).during, this.phaseActions(t.actions).after).some((a) => a.type === 'request_approval' && a.config?.allowStarterParticipants === true));
    if (startParticipantRules && !starterParticipantAllowed) throw new ForbiddenException('This workflow does not allow the starter to choose participants');
    if (starterParticipantAllowed && workflow.mode === 'MANUAL' && !startParticipantRules?.length) {
      const hasApproval = definition.transitions.some((t) => this.phaseActions(t.actions).before.concat(this.phaseActions(t.actions).during, this.phaseActions(t.actions).after).some((a) => a.type === 'request_approval' && a.config?.allowStarterParticipants === true));
      if (hasApproval) throw new BadRequestException('Select at least one workflow participant');
    }
    if (startParticipantRules) {
      const supported = startParticipantRules.every((r) => r && ['USER','GROUP','ROLE'].includes(String(r.type)) && (Array.isArray(r.ids) || Array.isArray(r.roles)));
      if (!supported) throw new BadRequestException('Invalid workflow participant selection');
