...
export class WorkflowsService {
  constructor(private readonly prisma: PrismaService, private readonly engine: WorkflowEngineService, private readonly functionExecutor: CustomFunctionExecutor) {}
  ...
  async list(user: AccessTokenPayload, scope?: string) {
    return this.prisma.workflow.findMany({ where: { orgId: user.org_id, ...(scope === 'mine' ? { ownerId: user.sub } : {}), ...(scope === 'drafts' ? { status: 'DRAFT' } : {}) }, include: { steps: { orderBy: { position: 'asc' } }, states: { orderBy: { position: 'asc' } }, transitions: true, activeVersion: { select: { id: true, version: true, publishedAt: true } } }, orderBy: { updatedAt: 'desc' } });
  }
  ...
  async ensureDefaults(user: AccessTokenPayload) {
    const defaults = [
      { name: 'Review uploaded documents', description: 'Create a review task for uploaded documents.', trigger: ['upload'], condition: { field: 'fileType', operator: 'equals', value: 'DOCUMENT' } },
      { name: 'Approval for PDFs', description: 'Request approval when a PDF is uploaded.', trigger: ['upload'], condition: { field: 'fileType', operator: 'equals', value: 'PDF' } },
      { name: 'Review and approval', description: 'Two-step review and approval template.', trigger: ['upload'], condition: 'any' },
    ];
    const result = [];
    for (const d of defaults) {
      ...
      const created = await this.prisma.$transaction(async (tx) => {
        const id = randomUUID();
        await tx.workflow.create({ data: { id, orgId: user.org_id, ownerId: user.sub, name: d.name, description: d.description, mode: 'AUTOMATIC', resourceType: 'FILE', isSystemDefault: true, calendarConfig: { timezone: 'UTC', workingDays: [1,2,3,4,5], workStart: '09:00', workEnd: '17:00', holidays: [] }, status: 'DRAFT', steps: { create: this.stepData({ ...d, actions: [], status: 'DRAFT', fields: [], states: [], transitions: [] } as any) } } });
        const s1=await tx.workflowState.create({data:{id:randomUUID(),workflowId:id,name:'Review',position:0,terminal:false}});
        const s2=await tx.workflowState.create({data:{id:randomUUID(),workflowId:id,name:'Completed',position:1,terminal:true}});
        await tx.workflowTransition.create({data:{id:randomUUID(),workflowId:id,fromStateId:s1.id,toStateId:s2.id,name:'Complete',trigger:'manual',condition:Prisma.JsonNull,actions:{before:[],during:[{type:'request_approval',config:{assigneeMode:'owner',approvalPolicy:'ANY',dueInMinutes:1440,reminderInMinutes:720}}],after:[]}}});
        return tx.workflow.findUnique({where:{id}});
      });
      result.push(created);
    }
    return result;
