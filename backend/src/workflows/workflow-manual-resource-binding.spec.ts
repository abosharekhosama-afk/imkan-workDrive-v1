...
describe('manual workflow resource binding regression', () => {
  it('keeps file and folder resource ids explicit in the manual-start contract', () => {
    ...
    expect(source).toContain("const resourceId = String(event.resourceId ?? event.fileId ?? '').trim();");
    ...
    expect(source).toContain("fileId: resourceId, resourceId, eventType: 'manual'");
  });
