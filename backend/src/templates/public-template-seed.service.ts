...
type CatalogItem = {
  ...
  type: TemplateType; file: string; extension: string; mime: string;
};
    ...
    for (const item of manifest) {
      ...
      await this.prisma.$transaction(async (tx) => {
        const template = existing
          ? await tx.template.update({ where: { id: templateId }, data: { status: TemplateStatus.ACTIVE, deletedAt: null, description: item.description, type: item.type, ownerId: null, orgId: null } })
