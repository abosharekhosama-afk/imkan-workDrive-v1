...
export function parseCreateJobs(body: unknown): { folderId: string | null; files: Array<{ id: string; name?: string }> } {
  ...
  const folderId = record.folderId === null || record.folderId === undefined ? null : record.folderId;
  ...
  return { folderId, files: [...new Map(files.map((file) => [file.id, file])).values()] };
}
