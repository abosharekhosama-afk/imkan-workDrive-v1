...
export class CloudImportController {
  ...
  @Get('oauth/:provider/start') start(@CurrentUser() user: AccessTokenPayload, @Param('provider') provider: string, @Query('folderId') folderId?: string) { return this.cloud.beginOAuth(user, parseProvider(provider), folderId || null); }
  @Public() @Get('oauth/:provider/callback') async callback(@Param('provider') provider: string, @Query('code') code: string, @Query('state') state: string, @Res() response: Response) { const result = await this.cloud.oauthCallback(parseProvider(provider), code, state); response.redirect(`${result.frontend}/files?cloudImport=${encodeURIComponent(provider)}&folderId=${encodeURIComponent(result.folderId ?? '')}`); }
  @Get(':provider/files') files(@CurrentUser() user: AccessTokenPayload, @Param('provider') provider: string) { return this.cloud.listFiles(user, parseProvider(provider)); }
