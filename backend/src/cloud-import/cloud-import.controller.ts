...
export class CloudImportController {
  ...
  @Get(':provider/files') files(@CurrentUser() user: AccessTokenPayload, @Param('provider') provider: string, @Query('connectionId') connectionId?: string, @Query('parentId') parentId?: string, @Query('pageToken') pageToken?: string) { return this.cloud.listFiles(user, parseProvider(provider), connectionId || null, parentId || null, pageToken || null); }
