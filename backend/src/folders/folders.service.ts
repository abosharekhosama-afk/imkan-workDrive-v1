...
export class FoldersService {
  ...
  private dispatchWorkflowFolderEvent(user: AccessTokenPayload, eventType: string, folder: { id: string; name: string; parentId?: string | null }) {
    void this.workflowEngine.onFileEvent(user, { eventType, fileId: folder.id, name: folder.name, mimeType: null, fileType: 'OTHER', size: '0', userId: user.sub, folderId: folder.parentId ?? null, resourceType: 'FOLDER' }).catch(() => undefined);
  }
  ...
  async rename(user: AccessTokenPayload, id: string, name: string) {
    ...
    this.dispatchWorkflowFolderEvent(user, 'properties_updated', updated);
    return updated;
  }
