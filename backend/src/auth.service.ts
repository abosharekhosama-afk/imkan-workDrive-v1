...
export class AuthService {
  ...
  async signup(input: { name: string; email: string; password: string; inviteToken?: string }): Promise<AuthResult> {
    ...
    if (input.inviteToken) {
      ...
      const invitation = await this.prisma.organizationInvitation.findFirst({ where: { tokenHash, email, acceptedAt: null, revokedAt: null, expiresAt: { gt: new Date() } } });
      if (!invitation) throw new BadRequestException('Invitation is invalid, expired, or does not match this email');
      const user = await this.prisma.$transaction(async (tx) => {
        const created = await tx.user.create({ data: { orgId: invitation.orgId, email, name, passwordHash, role: invitation.role } });
        await tx.organizationInvitation.update({ where: { id: invitation.id }, data: { acceptedAt: new Date(), status: InvitationStatus.ACCEPTED } });
        await tx.auditLog.create({ data: { orgId: invitation.orgId, actorId: created.id, action: 'ORG_INVITATION_ACCEPTED', resourceType: 'ORGANIZATION_INVITATION', resourceId: invitation.id } });
        return created;
      });
      return this.issue(user);
    }
    const organization = await this.prisma.organization.create({ data: { name: `${name}'s Workspace` } });
    const user = await this.prisma.user.create({ data: { orgId: organization.id, email, name, passwordHash, role: 'ADMIN' } });
    return this.issue(user);
  }
