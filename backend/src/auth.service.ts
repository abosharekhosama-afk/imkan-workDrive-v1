...
export class AuthService {
  ...
  async switchOrganization(user: AccessTokenPayload, organizationId: string): Promise<AuthResult> {
    ...
    const fullUser = await this.prisma.user.findUnique({ where: { id: user.sub } });
  ...
  async me(user: AccessTokenPayload) {
    ...
    const found = await this.prisma.user.findUnique({ where: { id: user.sub }, select: { id: true, name: true, email: true, status: true, avatarUrl: true, currentOrganizationId: true } });
    if (!found) throw new UnauthorizedException('Session is no longer valid');
    return { ...found, org_id: user.org_id, role: membership.role, membershipId: membership.id, membershipStatus: membership.status };
  }
