...
export function listDependencies(formula:string):string[]{if(!formula)return [];const out:string[]=[];const re=/(?:(?:'[^']+'|[A-Za-z_][\w ]*)!)?\$?[A-Z]+\$?\d+/gi;for(const m of formula.matchAll(re))out.push(m[0]);return [...new Set(out)]}
