...
export function addPivotTable(w:Workbook,sourceRange:string,name:string,rowField?:string,valueField?:string,aggregation:'sum'|'count'|'average'|'min'|'max'='sum'){const n=cloneWorkbook(w),s=activeSheet(n);if(!s)return w;s.pivotTables=[...(s.pivotTables??[]),{id:`pivot-${Date.now()}`,name:name.trim().slice(0,80)||`Pivot${(s.pivotTables?.length??0)+1}`,sourceRange:sourceRange.toUpperCase(),rowField:rowField?.trim(),valueField:valueField?.trim(),aggregation}];return n}
