...
export function splitParagraph(doc: WriterDocument, id: string, leftRuns: WriterRun[], rightRuns: WriterRun[]): WriterDocument { const next=cloneWriterDocument(doc); const index=next.blocks.findIndex(x=>x.id===id); if(index<0)return next; const source=next.blocks[index]; const left={...source,id:source.id,runs:leftRuns.length?leftRuns:[{text:''}]}; const right={...source,id:crypto.randomUUID(),runs:rightRuns.length?rightRuns:[{text:''}]}; if(source.type==='list-item' && source.ordered===undefined) right.ordered=false; next.blocks.splice(index,1,left,right); return next; }
