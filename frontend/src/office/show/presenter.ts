...
export const presenterProgress=(doc:ShowDocument,index:number)=>doc.slides.length?((clampSlideIndex(doc,index)+1)/doc.slides.length)*100:0;
export const presenterRemaining=(doc:ShowDocument,index:number)=>Math.max(0,doc.slides.length-clampSlideIndex(doc,index)-1);
export const presenterNextIndex=(doc:ShowDocument,index:number)=>{const n=index+1;return n<doc.slides.length?n:(doc.presenter?.loop?0:-1)};
export const presenterPrevIndex=(doc:ShowDocument,index:number)=>Math.max(0,index-1);
export const rehearsalDuration=(doc:ShowDocument)=>doc.slides.reduce((sum,s)=>sum+Math.max(0,s.autoAdvanceMs||0),0);
