...
import { getEnterpriseDashboard, getSecurityPolicy, getRetentionPolicy, type EnterpriseDashboard } from "@/lib/api/enterprise";
...
export default function AdminPage() {
  ...
  useEffect(() => {
    Promise.all([getAdminOverview(), getEnterpriseDashboard(), listAdminUsers(), listTeamFolders(), listCollaborationActivity(120), getSecurityPolicy(), getRetentionPolicy(), getStorageOverview()])
      .then(([o, d, u, tf, a, p, r, q]) => { setOverview(o); setEnterprise(d); setUsers(u); setTeamFolders(tf.teamFolders ?? []); setActivity(a); setPolicy(p); setRetention(r); setQuota(q); })
  ...
  return <main className="h-full overflow-y-auto bg-[#f7f7f7]" dir={ar ? "rtl" : "ltr"}>
    ...
    <div className="mx-auto max-w-[1420px] space-y-6 p-5 lg:p-7">
      ...
      <section className="grid gap-5 lg:grid-cols-3"><Link href="/admin/workflows" className="rounded-2xl border bg-white p-5 hover:border-[#9bb9f5]"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#edf3ff] text-[#2c66dd]"><Icons.flow size={19}/></div><h3 className="mt-4 text-[13px] font-semibold">{ar?'سير العمل':'Workflows'}</h3><p className="mt-1 text-[10px] leading-5 text-slate-500">{ar?'افتح واجهة سير العمل الحالية بنفس منطق التنفيذ الفعلي.':'Open the existing workflow UI backed by the real runtime.'}</p></Link><Link href="/admin/custom-functions" className="rounded-2xl border bg-white p-5 hover:border-[#9bb9f5]"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#edf3ff] text-[#2c66dd]"><Icons.code size={19}/></div><h3 className="mt-4 text-[13px] font-semibold">{ar?'الدوال المخصصة':'Custom Functions'}</h3><p className="mt-1 text-[10px] leading-5 text-slate-500">{ar?'واجهة مستقلة في Admin Console مع الحفاظ على SAFE runtime الحالي.':'A standalone Admin Console surface using the existing SAFE runtime.'}</p></Link><Link href="/admin/connections" className="rounded-2xl border bg-white p-5 hover:border-[#9bb9f5]"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#edf3ff] text-[#2c66dd]"><Icons.link size={19}/></div><h3 className="mt-4 text-[13px] font-semibold">{ar?'الاتصالات':'Connections'}</h3><p className="mt-1 text-[10px] leading-5 text-slate-500">{ar?'إدارة OAuth والاتصالات الموجودة بدون إنشاء منطق جديد.':'Manage the existing OAuth and connection implementation.'}</p></Link></section>

      <section className="grid gap-5 lg:grid-cols-3"><div className="rounded-2xl border bg-white p-5"><h3 className="text-[13px] font-semibold">{ar?'سياسة الأمان':'Security policy'}</h3><div className="mt-3 space-y-2 text-[10.5px] text-slate-600"><div className="flex justify-between"><span>MFA</span><b>{policy?.requireMfa ? (ar?'مطلوب':'Required') : (ar?'اختياري':'Optional')}</b></div><div className="flex justify-between"><span>{ar?'المشاركة الخارجية':'External sharing'}</span><b>{policy?.allowExternalSharing ? (ar?'مسموحة':'Allowed') : (ar?'محظورة':'Blocked')}</b></div><div className="flex justify-between"><span>{ar?'الروابط العامة':'Public links'}</span><b>{policy?.allowPublicLinks ? (ar?'مسموحة':'Allowed') : (ar?'محظورة':'Blocked')}</b></div></div><Link href="/admin/settings" className="mt-4 inline-block text-[10px] font-semibold text-[#175cd3]">{ar?'إدارة السياسة':'Manage policy'}</Link></div><div className="rounded-2xl border bg-white p-5"><h3 className="text-[13px] font-semibold">{ar?'الاحتفاظ':'Retention'}</h3><p className="mt-3 text-[11px] text-slate-600">{retention?.trashDays ?? '—'} {ar?'يوم لسلة المهملات':'days for trash'} · {retention?.deletedItemsDays ?? '—'} {ar?'يوم للعناصر المحذوفة':'days for deleted items'}</p><Link href="/admin/settings" className="mt-4 inline-block text-[10px] font-semibold text-[#175cd3]">{ar?'إعدادات الاحتفاظ':'Retention settings'}</Link></div><div className="rounded-2xl border bg-white p-5"><h3 className="text-[13px] font-semibold">{ar?'الحماية':'Protection'}</h3><p className="mt-3 text-[11px] text-slate-600">{ar?'إدارة DLP والجلسات وسجلات التدقيق من الشريط الجانبي.':'Manage DLP, devices, and audit logs from the admin sidebar.'}</p><div className="mt-4 flex flex-wrap gap-2"><Link href="/admin/dlp" className="rounded-lg bg-slate-100 px-3 py-1.5 text-[10px] font-semibold">DLP</Link><Link href="/admin/devices" className="rounded-lg bg-slate-100 px-3 py-1.5 text-[10px] font-semibold">{ar?'الأجهزة':'Devices'}</Link><Link href="/admin/audit" className="rounded-lg bg-slate-100 px-3 py-1.5 text-[10px] font-semibold">Audit</Link></div></div></section>
    </div>
  </main>;
}
