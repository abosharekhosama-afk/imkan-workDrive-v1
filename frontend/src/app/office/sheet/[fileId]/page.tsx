...
import { addConditionalFormat, addNamedRange, addPivotTable, addSheet, addTable, clearFilter, deleteActiveSheet, deleteColumns, deleteRows, freeze, insertColumns, insertRows, mergeRange, patchFormat, renameSheet, setActiveSheet, setColumnWidth, setFilter, setRowHeight, setValidation, shiftFormulaReferences, sortSheet, unmergeRange, updateCell } from '@/office/sheet/commands';
...
import { clearRange, patchRangeFormat, setRangeValidation, toggleFreeze } from '@/office/sheet/phase2-commands';
...
export default function SheetPage() {
  ...
  return <div id="office-main" dir={ar?'rtl':'ltr'} className="flex h-screen flex-col bg-slate-100 text-slate-800">
    ...
    <header className="flex h-14 items-center justify-between border-b bg-white px-4"><div className="flex items-center gap-3"><button onClick={()=>router.back()} className="rounded px-3 py-2 text-xs hover:bg-slate-100">← {t('Back','رجوع')}</button><div><div className="text-sm font-semibold">{doc.title}</div><div className="text-[10px] text-slate-400">{saving?t('Syncing…','جارٍ المزامنة…'):saved?t('Saved','تم الحفظ'):t('Unsynced changes','تغييرات غير متزامنة')} · Revision {revision}{offlineQueueCount(fileId)>0?` · ${offlineQueueCount(fileId)} queued`:''}{typeof navigator!=='undefined'&&!navigator.onLine?t(' · Offline',' · غير متصل'):''}{remoteRevision&& !saved ? t(' · Remote edit pending',' · تعديل بعيد بانتظار المزامنة'):''}</div></div></div><div className="flex items-center gap-2"><OfficePresenceView items={presence} ar={ar}/><div className="flex gap-1"><button className="rounded border px-2 py-1 text-xs disabled:opacity-40" disabled={!history.length} onClick={undo}>↶</button><button className="rounded border px-2 py-1 text-xs disabled:opacity-40" disabled={!future.length} onClick={redo}>↷</button><OfficeTransferControls fileId={fileId} type="SHEET" ar={ar}/><button onClick={()=>persist(ref.current!)} className="rounded bg-[var(--wd-primary)] px-3 py-2 text-xs font-medium text-white">{t('Save','حفظ')}</button></div></div></header>
    ...
    <SheetToolbar tab={sheetTab} ar={ar} gridlines={gridlines} onTabChange={setSheetTab} onUndo={undo} onRedo={redo} onSave={saveNow} onPrint={()=>window.print()} onExport={exportXlsx} onFind={findCell} onGridlinesToggle={()=>setGridlines(v=>!v)} onCopy={()=>void copy()} onCut={()=>void cut()} onPaste={()=>void paste()} onClear={clearSelected} onFreezeToggle={()=>commit(toggleFreeze(doc,1,1),true)} cell={selectedCell} onBold={()=>commit(patchRangeFormat(doc,bounds.start,bounds.end,{bold:!selectedCell?.format?.bold}))} onItalic={()=>commit(patchRangeFormat(doc,bounds.start,bounds.end,{italic:!selectedCell?.format?.italic}))} onUnderline={()=>commit(patchRangeFormat(doc,bounds.start,bounds.end,{underline:!selectedCell?.format?.underline}))} onStrike={()=>commit(patchRangeFormat(doc,bounds.start,bounds.end,{strike:!selectedCell?.format?.strike}))} onColor={v=>commit(patchRangeFormat(doc,bounds.start,bounds.end,{color:v}))} onBg={v=>commit(patchRangeFormat(doc,bounds.start,bounds.end,{background:v}))} onAlign={v=>commit(patchRangeFormat(doc,bounds.start,bounds.end,{align:v}))} onFormat={v=>commit(patchRangeFormat(doc,bounds.start,bounds.end,{numberFormat:v}))} onSort={()=>commit(sortSheet(doc,selected.replace(/\d+$/,''),'asc'),true)} onFilter={()=>{const col=selected.replace(/\d+$/,'');const q=window.prompt(t('Filter value (empty clears)','قيمة التصفية (فارغة للإلغاء)'),sheet.filters?.[col]??'');commit(q?setFilter(doc,col,q):clearFilter(doc,col),true);}} onValidation={()=>{const raw=window.prompt(t('Validation list values separated by commas, or empty to remove','قيم القائمة مفصولة بفواصل، أو فارغ للحذف'),(selectedCell?.validation?.values??[]).join(','));commit(raw?setValidation(doc,selected,{type:'list',values:raw.split(',').map(x=>x.trim()).filter(Boolean).slice(0,50)}):setValidation(doc,selected,undefined),true);}} onConditional={addConditional} onMerge={()=>commit(mergeRange(doc,bounds.start,bounds.end),true)} onNamedRange={addNewNamedRange} onTable={addNewTable} onPivot={addNewPivot} onUnmerge={()=>commit(unmergeRange(doc,bounds.start,bounds.end),true)} onAddSheet={()=>commit(addSheet(doc),true)} onDeleteSheet={()=>commit(deleteActiveSheet(doc),true)} onRename={()=>{const n=window.prompt(t('Sheet name','اسم الورقة'),sheet.name);if(n)commit(renameSheet(doc,n),true);}} onFreeze={()=>commit(freeze(doc,1,1),true)} onChart={addNewChart} onInsertRows={()=>{const p=parseKey(selected)!;commit(insertRows(doc,p.row,1),true)}} onDeleteRows={()=>{const p=parseKey(selected)!;commit(deleteRows(doc,p.row,1),true)}} onInsertColumns={()=>{const p=parseKey(selected)!;commit(insertColumns(doc,p.col,1),true)}} onDeleteColumns={()=>{const p=parseKey(selected)!;commit(deleteColumns(doc,p.col,1),true)}} onHelp={()=>window.alert(t('IMKAN Sheet\nZoho-like spreadsheet workspace with real editing, formulas, tables, pivots, charts, filters, validation, freeze panes, collaboration and XLSX export.','IMKAN Sheet\nواجهة جداول بيانات شبيهة بـ Zoho مع تحرير وصيغ وجداول وجداول محورية ورسوم ومرشحات وتحقق وتجميد وتعاون وتصدير XLSX.'))}/>
    <div className="flex min-h-10 items-center gap-2 border-b bg-white px-3 py-1.5 text-xs shadow-sm"><div className="flex h-8 w-20 items-center rounded border border-slate-300 bg-white px-2 font-mono font-semibold text-slate-700"><span>{bounds.start===bounds.end?selected:`${bounds.start}:${bounds.end}`}</span></div><div className="flex h-8 w-8 items-center justify-center text-sm font-semibold text-slate-400">fx</div><div className="flex min-w-0 flex-1 items-center rounded border border-slate-300 bg-white px-3 shadow-inner">{editing?<input aria-label={t('Formula bar','شريط الصيغة')} autoFocus className="h-7 w-full outline-none" value={input} onChange={e=>setInput(e.target.value)} onBlur={finish} onKeyDown={e=>e.key==='Enter'&&finish()}/>:<button className="h-7 w-full truncate text-left text-slate-700" onDoubleClick={()=>begin(selected)}>{display||t('Enter value or formula','أدخل قيمة أو صيغة')}</button>}</div><button className="h-8 rounded border border-slate-300 bg-white px-3 font-medium hover:bg-slate-50" onClick={fillDown}>{t('Fill Down','تعبئة لأسفل')}</button></div>
