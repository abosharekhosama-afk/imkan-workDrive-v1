...
type ScopeDef = { value: string; label: string; description: string };
type ServiceCard = {
  key: string;
  name: string;
  provider: string;
...
export default function ConnectionsPage() {
  ...
  const scopeOptions = serviceProvider?.scopes?.length ? serviceProvider.scopes : (selectedService?.scopes ?? []);
  const configured = serviceProvider?.configured !== false;
  ...
  const beginCreate = (service: ServiceCard) => {
    ...
    setDraft({ name: "", linkName: service.provider, authType: meta?.authTypes?.[0] ?? service.authType ?? "API_KEY", parameterKey: "", parameterLabel: "", secret: "", username: "", password: "", baseUrl: meta?.baseUrl ?? service.baseUrl ?? "", scopes: [...(meta?.defaultScopes ?? service.defaultScopes ?? [])] });
  ...
  const create = async () => {
    ...
    try {
      ...
      const authType = provider === "rest" ? draft.authType : (providerMap.get(provider)?.authTypes?.[0] ?? (selectedService.oauth ? "OAUTH2" : draft.authType));
      if (selectedService.oauth && !configured) throw new Error(`${selectedService.name} OAuth is not configured on the server. Configure the provider OAuth client variables before connecting.`);
            ...
            <div className="px-8 pb-10 pt-7">
              ...
              <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
                {visibleServices.map((service) => {
                  ...
                  const meta = providerMap.get(service.provider);
                  const isConfigured = !service.oauth || meta?.configured !== false;
          ...
          <>
            ...
            <div className="px-8 py-6 pb-12">
              <div className="overflow-hidden rounded-[5px] border border-[#DCE4EC]">
                ...
                {accordion === "service" ? <div className="border-t border-[#E7EDF2] bg-white p-6"><div className="mb-5 flex items-center gap-4"><ServiceLogo service={selectedService} size={44} /><span className="text-[17px] font-medium text-[#0B63E5]">{selectedService.name}</span></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-3">{SERVICE_CATALOG.map((s) => <button key={s.key} type="button" onClick={() => { const meta = providerMap.get(s.provider); setSelectedService(s); setDraft((d) => ({ ...d, authType: meta?.authTypes?.[0] ?? s.authType ?? "API_KEY", linkName: s.provider, baseUrl: meta?.baseUrl ?? s.baseUrl ?? "", scopes: [...(meta?.defaultScopes ?? s.defaultScopes ?? [])] })); setAccordion("details"); }} className={`flex items-center gap-3 rounded-[5px] border px-4 py-3 text-start transition ${selectedService.key === s.key ? "border-[#0B63E5] bg-[#F4F8FF]" : "border-[#E2E7EE] hover:border-[#B9C7D8]"}`}><ServiceLogo service={s} size={44} /><span><span className="block text-[12px] text-[#7B8794]">SERVICE NAME</span><span className="block text-[15px] text-[#1F2937]">{s.name}</span></span></button>)}</div></div> : null}
              </div>
              ...
              <div className="mt-3 overflow-hidden rounded-[5px] border border-[#DCE4EC]">
                ...
                {accordion === "details" ? <div className="border-t border-[#E7EDF2] p-7"><div className="grid grid-cols-1 gap-6 md:grid-cols-2"><label className="block"><span className="mb-2 block text-[14px] font-semibold text-[#1F2937]">Connection Name</span><input value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} placeholder="Enter Connection Name" className="h-[54px] w-full rounded-[5px] border border-[#D8E0E8] px-4 text-[14px] outline-none focus:border-[#0B63E5] focus:ring-2 focus:ring-[#0B63E5]/10" /></label><label className="block"><span className="mb-2 block text-[14px] font-semibold text-[#1F2937]">Connection Link Name</span><input value={draft.linkName} onChange={(e) => setDraft({ ...draft, linkName: e.target.value })} placeholder="Enter Connection Link Name" className="h-[54px] w-full rounded-[5px] border border-[#D8E0E8] px-4 text-[14px] outline-none focus:border-[#0B63E5] focus:ring-2 focus:ring-[#0B63E5]/10" /></label></div>
                  <div className="mt-6">
                    ...
                    {scopeOptions.length ? <div className="mt-6"><div className="mb-2 text-[14px] font-semibold text-[#1F2937]">Choose Scopes</div><div className="max-h-[280px] overflow-y-auto rounded-[5px] border border-[#D8E0E8]">{scopeOptions.map((sc) => <label key={sc.value} className="flex cursor-pointer items-start gap-3 border-b border-[#EEF1F4] px-4 py-3 last:border-0 hover:bg-[#FAFCFE]"><input type="checkbox" className="mt-1 h-4 w-4 accent-[#0B63E5]" checked={draft.scopes.includes(sc.value)} onChange={(e) => setDraft((d) => ({ ...d, scopes: e.target.checked ? [...new Set([...d.scopes, sc.value])] : d.scopes.filter((x) => x !== sc.value) }))} /><span><span className="block text-[13px] font-medium text-[#1F2937]">{sc.label}</span><span className="mt-0.5 block text-[11px] leading-5 text-[#7A8491]">{sc.description}</span><code className="mt-1 block break-all text-[9px] text-[#98A2B3]">{sc.value}</code></span></label>)}</div><div className="mt-2 text-[10px] text-[#7A8491]">Only the selected permissions are requested from the provider.</div></div> : null}
                  </div>
