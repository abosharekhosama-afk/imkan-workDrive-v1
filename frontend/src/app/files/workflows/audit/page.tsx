          ...
          <section className="wf-card mt-6 wd-card-body ">
            ...
            <div className="mt-3 grid gap-2 md:grid-cols-3">
              <input
                ...
                placeholder={ar ? "Workflow ID" : "Workflow ID"}
              />
              <select className="wf-input" value={v1} onChange={(e) => setV1(e.target.value)}>
                {versions.length === 0 ? (
                  <option value="">{ar ? "الإصدار الأول" : "Version A"}</option>
                ) : (
                  versions.map((v) => (
              ...
              <select className="wf-input" value={v2} onChange={(e) => setV2(e.target.value)}>
                {versions.length === 0 ? (
                  <option value="">{ar ? "الإصدار الثاني" : "Version B"}</option>
                ) : (
                  versions.map((v) => (
                    <option key={v.id} value={v.id}>
                      v{v.version}
            ...
            <button
              className="wf-primary-button mt-3"
              disabled={!workflowId}
              onClick={() =>
                void listWorkflowVersions(workflowId)
                  .then((v) => {
                    setVersions(v);
                    setV1(v[1]?.id || v[0]?.id || "");
                    setV2(v[0]?.id || "");
            ...
            {versions.length > 0 && (
              <>
                ...
                <button
                  ...
                  onClick={() =>
                    void Promise.all([getWorkflowVersionDetail(workflowId, v1), getWorkflowVersionDetail(workflowId, v2)])
                      .then(([a, b]) => {
                        setD1(a);
                        setD2(b);
                        const flatten = (x: any, prefix = ""): Array<[string, unknown]> => {
                          if (x === null || typeof x !== "object") return [[prefix, x]];
                          if (Array.isArray(x)) return x.flatMap((v, i) => flatten(v, `${prefix}[${i}]`));
                          return Object.entries(x).flatMap(([k, v]) => flatten(v, prefix ? `${prefix}.${k}` : k));
                        };
                        ...
                        setDiff(
                          [...keys]
                            .filter((k) => JSON.stringify(aa.get(k)) !== JSON.stringify(bb.get(k)))
