...
function ActionEditor({ actions, onChange, resourceType, ar }: { actions: Action[]; onChange: (next: Action[]) => void; resourceType: string; ar: boolean }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([listWorkflowParticipants(), listWorkflowParticipantOptions(), listWorkflowTemplates(), listWorkflowFunctions()]).then(([p, po, t, f]) => {
      if (!alive) return;
      setParticipants(p.map(item => item.user));
  ...
  return <div ref={canvasRef} className="workflow-canvas-pane relative h-full min-h-0 overflow-auto bg-[#F8FAFC]" dir="ltr">
    ...
    <div className="relative" style={{ width: width * zoom, height: height * zoom }}>
      ...
      {states.map((s, i) => <div key={i} onPointerDown={(e) => { const target=e.target as HTMLElement; if (target.closest("button, .workflow-connection-handle")) return; const point=pointerPoint(e); setDrag({index:i,dx:point.x-positions[i].x,dy:point.y-positions[i].y}); }} style={{ left: positions[i].x * zoom, top: positions[i].y * zoom, transform:`scale(${zoom})`, transformOrigin:"top left" }} className="absolute z-20 w-[270px] select-none overflow-visible">
        <button type="button" onClick={() => onSelect(`state-${i}`)} className={`w-full rounded-lg border bg-white p-4 text-start shadow-[0_8px_25px_rgba(15,23,42,.08)] transition hover:-translate-y-0.5 ${selected === `state-${i}` ? "border-[#1B66EA] ring-4 ring-[#1B66EA]/10" : "border-slate-200"}`} dir={ar?"rtl":"ltr"}>
...
export default function WorkflowBuilderPage() {
  ...
  return <div className="workflow-ui flex h-full min-h-0 flex-col bg-[#F7F9FC]" dir={ar ? "rtl" : "ltr"}>
    <header className="workflow-builder-header shrink-0 bg-white px-5 py-3"><div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><Link href="/files/workflows" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50">←</Link><div className="min-w-0"><div className="text-[9px] font-semibold uppercase tracking-[.17em] text-[#1B66EA]">{txt(ar,"Workflow builder","منشئ سير العمل")}</div><div className="truncate text-[15px] font-semibold text-slate-900">{name || txt(ar,"Create workflow","إنشاء سير عمل")} <span className="ms-2 text-[10px] font-medium text-slate-500">{mode === "MANUAL" ? txt(ar,"Manual","يدوي") : txt(ar,"Automatic","تلقائي")}</span></div></div></div><div className="flex items-center gap-1.5"><button type="button" onClick={() => setElementsOpen(true)} className="workflow-builder-mobile-menu wd-icon-btn xl:hidden" aria-label={txt(ar,"Open workflow elements","فتح عناصر سير العمل")} title={txt(ar,"Open workflow elements","فتح عناصر سير العمل")}>☰</button><WorkflowHelp compact /><label className="wd-pill wd-pill-new cursor-pointer">{txt(ar,"Import JSON","استيراد JSON")}<input type="file" accept="application/json" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) void importSpec(f); e.currentTarget.value = ""; }} /></label><button type="button" onClick={exportSpec} className="wd-pill wd-pill-new">{txt(ar,"Export JSON","تصدير JSON")}</button><button type="button" disabled={busy} onClick={() => void save("DRAFT")} className="wd-pill wd-pill-new">{txt(ar,"Save draft","حفظ المسودة")}</button><button type="button" disabled={busy} onClick={() => setActivationOpen(true)} className="wd-pill wd-pill-new">{txt(ar,"Activate","تفعيل")}</button></div></div>
    ...
    {step === 1 && <div className="min-h-0 flex-1 overflow-hidden p-4"><div className="grid h-full min-h-0 grid-cols-[260px_minmax(0,1fr)] gap-4">
      ...
      <section className="wd-card min-h-0 overflow-y-auto"><div className="border-b border-slate-100 px-6 py-5"><div className="flex items-start justify-between gap-4"><div><h2 className="text-[17px] font-semibold text-slate-900">{txt(ar,"Workflow fields","حقول سير العمل")}</h2><p className="mt-1 max-w-[720px] text-[11px] leading-5 text-slate-500">{txt(ar,"Collect feedback, assignees, approvers and other values while a workflow transition is in progress.","اجمع الملاحظات والمكلفين والموافقين والقيم الأخرى أثناء تنفيذ انتقال في سير العمل.")}</p></div><WorkflowHelp compact /></div></div>
        <div className="grid gap-4 p-6 lg:grid-cols-[1fr_310px]">
          <div onDragOver={(e) => { e.preventDefault(); setFieldsDragOver(true); }} onDragLeave={() => setFieldsDragOver(false)} onDrop={onDropField} className={`min-h-[430px] rounded-2xl border-2 border-dashed p-5 transition ${fieldsDragOver ? "border-[#1B66EA] bg-[#F8FBFF] scale-[1.005]" : "border-slate-200 bg-slate-50/50"}`}>
