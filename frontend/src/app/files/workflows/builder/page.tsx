...
import { ConnectionUseHint, WorkflowConceptGuide, WorkflowNextSteps } from "@/components/workflow-concept-guide";
import { WorkflowNodePalette, type WorkflowPaletteNode } from "@/components/workflow-node-palette";
...
function defaultWorkflowStart(): { states: StateDraft[]; transitions: TransitionDraft[]; positions: Position[] } {
  return { states: [{ name: "Start", description: "Workflow entry point", terminal: false }], transitions: [], positions: [{ x: 90, y: 100 }] };
}
function recipeDefaults(recipe: string): { states: StateDraft[]; transitions: TransitionDraft[]; positions: Position[] } {
...
function ActionEditor({ actions, onChange, resourceType, ar, workflowFields }: { actions: Action[]; onChange: (next: Action[]) => void; resourceType: string; ar: boolean; workflowFields: WorkflowField[] }) {
  ...
  useEffect(() => { void listConnections().then(setConnections).catch(() => undefined); }, []);
      ...
      >
        ...
        <div className="workflow-action-card-body">
          ...
          {a.type === "custom_function" && <>
            ...
            {linkedFunction
              ? <div className="workflow-function-link sm:col-span-2"><span className="workflow-function-link-icon">ƒ</span><div className="min-w-0 flex-1"><div className="truncate font-semibold">{linkedFunction.name}</div><div className="text-[9px] text-slate-500">{ar ? "الإصدار النشط سيُنفذ مع هذا الإجراء." : "The active version will execute with this action."}</div></div><span className="workflow-function-version">v{linkedFunction.activeVersion?.version ?? 1}</span><Link href={`${workflowBase}/functions`} className="text-[9px] font-semibold text-[var(--wd-primary)] hover:underline">{ar ? "إدارة الدوال" : "Manage functions"}</Link></div>
              : <div className="workflow-function-empty sm:col-span-2"><span>{functions.length ? (ar ? "اختر دالة آمنة لربط هذا الإجراء بها." : "Select a safe function to link this action.") : (ar ? "لا توجد دوال آمنة مخصصة بعد." : "No custom safe functions yet.")}</span><Link href={`${workflowBase}/functions`} className="font-semibold text-[var(--wd-primary)] hover:underline">{ar ? "إنشاء دالة" : "Create function"}</Link></div>}
          </>}
        </div>
      </div>;
    })}
...
export default function WorkflowBuilderPage() {
  ...
  const starter = defaultWorkflowStart();
  const canvasMountRef = useRef<HTMLDivElement>(null);
  useEffect(() => { if (urlId || access?.canCreate) return; router.replace(workflowBase); }, [access, router, urlId]); const [step, setStep] = useState<1 | 2 | 3>(1); const [name, setName] = useState(params.get("name") || ""); const [description, setDescription] = useState(params.get("description") || ""); const [mode, setMode] = useState<"AUTOMATIC" | "MANUAL">(params.get("mode") === "MANUAL" ? "MANUAL" : "AUTOMATIC"); const [resourceType, setResourceType] = useState<"FILE" | "FOLDER">(params.get("resourceType") === "FOLDER" ? "FOLDER" : "FILE");
  ...
  const addNodeTemplate = (template: WorkflowNodeTemplate, point?: Position) => {
    if (template === "trigger") {
      ...
      setSelected("state-0");
      if (mode === "MANUAL") {
        setMessage(txt(ar, "Manual workflows start from the Start state. Users launch them from file actions.", "سير العمل اليدوي يبدأ من حالة Start. يطلقه المستخدمون من إجراءات الملف."));
      } else {
        setMessage(txt(ar, "Configure starting triggers in the left panel under Starting trigger.", "اضبط محفزات البداية من اللوحة اليسرى ضمن محفز البداية."));
      }
      return;
    }
    ...
    const labels: Record<Exclude<WorkflowNodeTemplate, "trigger">, { name: string; description: string; terminal: boolean }> = {
      ...
      http_request: { name: txt(ar,"API request","طلب API"), description: txt(ar,"Call an external service using a connection.","استدعاء خدمة خارجية عبر اتصال."), terminal: false },
      custom_function: { name: txt(ar,"Function","دالة"), description: txt(ar,"Run a published safe function.","تشغيل دالة آمنة منشورة."), terminal: false },
      import_file: { name: txt(ar,"Import file","استيراد ملف"), description: txt(ar,"Read a file from a connected cloud provider.","قراءة ملف من مزود سحابي متصل."), terminal: false },
      end: { name: txt(ar,"End","نهاية"), description: txt(ar,"Terminal workflow state.","حالة نهائية لسير العمل."), terminal: true },
    };
    const spec = labels[template as Exclude<WorkflowNodeTemplate, "trigger">];
    const basePoint = positions[from] ?? { x: 120, y: 120 };
    const target = point ? { x: Math.max(20, point.x - 135), y: Math.max(30, point.y - 55) } : { x: basePoint.x + 330, y: basePoint.y + 35 };
    ...
    if (template === "http_request") { transition.name = txt(ar,"Call API","استدعاء API"); transition.during = [action("http_request", { connectionId: "", method: "GET", path: "/resource", responseMode: "JSON", maxResponseBytes: 20000 })]; }
    if (template === "custom_function") { transition.name = txt(ar,"Run function","تشغيل الدالة"); transition.during = [action("custom_function", { functionId: "" })]; }
    if (template === "import_file") { transition.name = txt(ar,"Import file","استيراد ملف"); transition.during = [action("connection_file", { connectionId: "", provider: "", resourceId: "", resourceName: "" })]; }
  ...
  return <div className="workflow-ui flex h-full min-h-0 flex-col bg-[#F7F9FC]" dir={ar ? "rtl" : "ltr"}>
    ...
    </div></aside></div><div className="workflow-canvas-status wd-card flex shrink-0 items-center justify-between gap-3 px-4 py-2" dir={ar ? "rtl" : "ltr"}><div className="flex items-center gap-3"><span className="text-[10px] text-slate-500">{states.length} {txt(ar,"states","حالات")} · {transitions.length} {txt(ar,"transitions","انتقالات")}</span><div className="hidden items-center gap-2 sm:flex"><span className="text-[9px] font-semibold uppercase tracking-[.14em] text-slate-400">{txt(ar,"Mini map","خريطة مصغرة")}</span><div className="relative h-[70px] w-[130px] cursor-crosshair overflow-hidden rounded-lg border border-slate-200 bg-slate-50" onClick={(e)=>{ const rect=(e.currentTarget as HTMLElement).getBoundingClientRect(); const nx=Math.max(0,Math.min(1,(e.clientX-rect.left)/rect.width)); const ny=Math.max(0,Math.min(1,(e.clientY-rect.top)/rect.height)); const el=canvasMountRef.current; if(el){ el.scrollLeft=Math.max(0,nx*canvasMetrics.width*zoom-el.clientWidth/2); el.scrollTop=Math.max(0,ny*canvasMetrics.height*zoom-el.clientHeight/2); } }}><div className="absolute inset-2 rounded border border-slate-200 bg-white">{states.map((s,i)=><span key={i} className={`absolute h-2 w-4 rounded-sm ${s.terminal?"bg-emerald-400":"bg-[var(--wd-primary)]"}`} style={{left:`${Math.min(86,(positions[i].x/canvasMetrics.width)*100)}%`,top:`${Math.min(86,(positions[i].y/canvasMetrics.height)*100)}%`}}/>)}</div></div></div></div><div className="flex items-center gap-2"><button type="button" onClick={() => setPaletteOpen(true)} className="wd-pill wd-pill-record lg:hidden">{txt(ar,"Palette","القائمة")}</button><button type="button" onClick={() => setInspectorOpen(true)} className="wd-pill wd-pill-record xl:hidden">{txt(ar,"Inspector","المفتش")}</button></div></div><div className="wd-card flex shrink-0 items-center justify-between px-4 py-3"><button type="button" onClick={() => setStep(1)} className="rounded-xl border border-slate-200 px-4 py-2 text-[10.5px] text-slate-600">{txt(ar,"Back","رجوع")}</button><button type="button" onClick={() => setStep(3)} className="rounded-xl bg-[var(--wd-primary)] px-5 py-2 text-[10.5px] font-semibold text-white">{txt(ar,"Continue to review","متابعة إلى المراجعة")} →</button></div></div></div>}
...
<div className="mt-5 rounded-xl border border-slate-200"><div className="border-b border-slate-100 px-4 py-3 text-[11px] font-semibold">{txt(ar,"Validation","التحقق")}</div>{validation.length ? <div className="divide-y divide-slate-100">{validation.map((x,i)=><div key={i} className="flex gap-2 px-4 py-3 text-[10.5px] text-amber-700"><span>!</span>{x}</div>)}</div> : <div className="flex items-center gap-2 px-4 py-5 text-[10.5px] text-emerald-700"><span className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-100">✓</span>{txt(ar,"All required checks passed. The workflow is ready for activation.","اجتازت جميع الفحوصات المطلوبة. سير العمل جاهز للتفعيل.")}</div>}</div><div className="mt-5 rounded-xl bg-[#F7FAFF] p-4 text-[10.5px] leading-5 text-slate-600">{txt(ar,"Automatic workflows react to configured file/folder events. Manual workflows become available from the file actions menu and create a waiting task when a manual transition is reached.","تستجيب سير العمل التلقائية لأحداث الملفات والمجلدات المحددة. أما سير العمل اليدوية فتظهر ضمن قائمة إجراءات الملف وتنشئ مهمة انتظار عند الوصول إلى انتقال يدوي." )}</div><div className="mt-6 flex justify-between"><button type="button" onClick={() => setStep(2)} className="wd-pill wd-pill-record">{txt(ar,"Back to design","العودة إلى التصميم")}</button><button type="button" disabled={busy || validation.length > 0} onClick={() => setActivationOpen(true)} className="wd-pill wd-pill-new disabled:opacity-40">{txt(ar,"Activate workflow","تفعيل سير العمل")}</button></div></div></div>}
    ...
    {activatedId && <div className="mx-5 mb-4"><WorkflowNextSteps ar={ar} workflowId={activatedId} /></div>}
