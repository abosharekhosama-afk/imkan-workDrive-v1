...
export default function QueuePage() {
  ...
  const runAction = (action: "retry" | "requeue" | "recover" | "dead-letter") => {
    if (!selected) return;
    setBusy(action);
    void workflowQueueAction(selected.id, action)
      .then(() => Promise.all([listWorkflowQueue(status), Promise.resolve()]))
      .then(([items]) => {
        setRows(items);
        setSelected(null);
      })
      ...
      <main className="h-full min-h-0 overflow-y-auto bg-white p-2 sm:p-3" dir={ar ? "rtl" : "ltr"}>
            ...
            <div className="min-w-[980px]">
              ...
              {rows.length === 0 ? (
                ...
                <div key={job.id} className="grid grid-cols-[1.5fr_1fr_110px_90px_110px_170px_1.3fr] items-center border-b border-slate-100/80 px-4 py-3 text-[10px]">
                  ...
                  <span className="truncate text-red-500" title={job.lastError ?? ""}>{job.lastError ?? "—"}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {selected && (
          <div className="fixed inset-0 z-[230] flex justify-end">
            ...
            <aside className="relative h-full w-[min(520px,96vw)] overflow-y-auto bg-white p-5 shadow-2xl" dir={ar ? "rtl" : "ltr"}>
              ...
              <div className="wd-card w-full mt-4 space-y-2 p-4 text-[10px]">
                ...
                <div>{ar ? "الأولوية" : "Priority"}: {selected.priority}</div>
                <div>{ar ? "التشغيل" : "Run at"}: {new Date(selected.runAt).toLocaleString()}</div>
                <div>Lease: {selected.leaseUntil ? new Date(selected.leaseUntil).toLocaleString() : "—"}</div>
                {selected.lastError && <pre className="whitespace-pre-wrap rounded-xl bg-red-50 p-3 text-[9px] text-red-700">{selected.lastError}</pre>}
              </div>
