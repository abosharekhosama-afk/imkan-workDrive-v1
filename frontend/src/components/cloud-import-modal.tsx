...
export function CloudImportHost() {
  ...
  const open = (folderId: string | null) => { setDetail({ folderId }); setProvider(null); setFiles([]); setSelected([]); setJobs([]); setError(''); void refreshProviders(); void loadRecentJobs(); };
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams(window.location.search); const connected = params.get('cloudImport');
    if (connected) { open(params.get('folderId') || null); window.history.replaceState({}, '', window.location.pathname); }
    return () => window.removeEventListener('workdrive:cloud-import', handler);
  }, []);

  const chooseProvider = async (next: CloudProvider) => {
    setProvider(next); setSelected([]); setError(''); setLoading(true);
    try { const state = providers.find(p => p.provider === next); if (!state?.connected) { const { url } = await cloudOAuthStart(next, detail?.folderId ?? null); window.location.assign(url); return; } setFiles(await listCloudFiles(next)); }
  ...
  const importSelected = async () => {
    ...
    try { const created = await createCloudImports(provider, detail.folderId, files.filter(f => selected.includes(f.id)).map(f => ({ id: f.id, name: f.name }))); setJobs(created); setSelected([]); window.dispatchEvent(new Event('workdrive:content-changed')); }
