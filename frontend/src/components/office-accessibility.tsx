...
export function OfficeAccessibility({ar=false,status='',targetId='office-main'}:OfficeAccessibilityProps){
 ...
 return <>
  ...
  <style jsx global>{`[data-office-reduced-motion="true"] *,[data-office-reduced-motion="true"] *::before,[data-office-reduced-motion="true"] *::after{scroll-behavior:auto!important;animation-duration:.001ms!important;animation-iteration-count:1!important;transition-duration:.001ms!important}`
  +` :where(button,a,input,textarea,select,[tabindex]):focus-visible{outline:2px solid #2563eb;outline-offset:2px}`
  +` .office-hit-target{min-width:44px;min-height:44px}`}</style>
 </>;
}
