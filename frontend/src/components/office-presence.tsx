...
export function OfficePresence({ items, ar = false }: { items: OfficePresence[]; ar?: boolean }) {
  ...
  return <div className="flex items-center gap-1.5" title={active.map(x => `${x.user.name || x.user.email}${x.cursor?.blockId ? ` · ${ar ? 'يحرر فقرة' : 'editing a block'}` : ''}`).join(', ')}>
    ...
    <span className="text-[11px] text-slate-500">{active.length} {ar ? 'محرر الآن' : active.length === 1 ? 'editor here' : 'editors here'}{active.some(x=>x.cursor?.blockId)?` · ${ar?'يحررون الآن':'editing now'}`:''}</span>
  </div>;
}
