...
export function WorkflowPicker({resourceType,resourceId,resourceName,onClose,onStarted}:{resourceType:"FILE"|"FOLDER";resourceId:string;resourceName:string;onClose:()=>void;onStarted:()=>void}){
 ...
 return <Modal title={label("menu.assignWorkflow")} onClose={onClose}><div className="space-y-3"><p className="text-[12px] text-[color:var(--wd-text-muted)]">{resourceName}</p>{error&&<div className="rounded-lg bg-red-50 px-3 py-2 text-[11px] text-red-700">{error}</div>}{loading?<p className="text-[12px] text-[color:var(--wd-text-muted)]">{label("common.loading")}</p>:rows.length===0?<p className="rounded-lg bg-[color:var(--wd-hover)] px-3 py-4 text-center text-[12px] text-[color:var(--wd-text-muted)]">No manual workflows are available.</p>:<div className="max-h-72 space-y-2 overflow-y-auto">{rows.map(w=><button key={w.id} disabled={busy===w.id} onClick={()=>void start(w)} className="w-full rounded-lg border border-[color:var(--wd-line)] p-3 text-start hover:border-[color:var(--wd-primary)] hover:bg-[#F7FAFF] disabled:opacity-50"><div className="text-[12px] font-semibold">{w.name}</div><div className="mt-1 text-[10.5px] text-[color:var(--wd-text-muted)]">{w.description||"Manual workflow"}</div></button>)}</div>}</div></Modal>
}
