...
export function WorkflowPicker({resourceType,resourceId,resourceName,onClose,onStarted}:{resourceType:"FILE"|"FOLDER";resourceId:string;resourceName:string;onClose:()=>void;onStarted:()=>void}){
 ...
 const start=async()=>{
  ...
  try{
    const run=await startWorkflow(detail.id,{
      fileId:resourceId,
      resourceId,
      ...
      setStartedRun({id:run.id,status:run.status});
      onStarted();
 ...
 return <Modal title={label("menu.assignWorkflow")} onClose={onClose}><div className="space-y-3">
   ...
   :<><div className="wd-card flex items-start gap-2 bg-slate-50 p-3"><button onClick={(event)=>{event.preventDefault();event.stopPropagation();setDetail(null)}} className="wd-pill wd-pill-record">←</button><div className="min-w-0"><div className="text-[13px] font-semibold text-slate-900">{detail.name}</div><div className="mt-1 text-[10px] text-slate-500">{detail.description||"Manual workflow"}</div></div></div>
    ...
    {startedRun ? <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3">
      <div className="text-[10px] font-semibold text-emerald-800">Workflow run queued successfully</div>
      <div className="mt-1 text-[9.5px] text-emerald-700">Run ID: <span className="font-mono">{startedRun.id}</span> · {startedRun.status}</div>
      <div className="mt-3 flex flex-wrap gap-2">
        <Link href={`/files/workflows/runs?workflowId=${encodeURIComponent(detail.id)}`} className="wd-pill wd-pill-new">Open run history</Link>
        <button type="button" onClick={onClose} className="wd-pill wd-pill-record">Close</button>
      </div>
    </div> : <div className="flex justify-end gap-2"><button onClick={(event)=>{event.preventDefault();event.stopPropagation();setDetail(null)}} disabled={busy} className="wd-pill wd-pill-record">Cancel</button><button onClick={(event)=>{event.preventDefault();event.stopPropagation();void start()}} disabled={busy||!detail} className="wd-pill wd-pill-new disabled:opacity-50">{busy?"Starting…":"Start workflow"}</button></div>}
   </>}
 </div></Modal>;
}
