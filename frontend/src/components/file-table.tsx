          ...
          {sortedFolders.map((folder) => (
            <tr key={folder.id} draggable={Boolean(canMutate)} onDoubleClick={() => onOpen?.("FOLDER", folder.id, folder.name)} onContextMenu={(e) => { e.preventDefault(); setCtxMenu({ x: e.clientX, y: e.clientY, node: (<FileContextMenu
              handlers={{
                ...
                onDelete: canMutate ? () => onDelete("FOLDER", folder.id) : undefined, onAssignWorkflow: onAssignWorkflow ? () => onAssignWorkflow("FOLDER", folder.id, folder.name) : undefined,
              }}
              ...
              <td className="px-3 py-2 text-end">
                <FileActionsMenu
                  ...
                  handlers={{
                    ...
                    onDelete: canMutate ? () => onDelete("FILE", file.id) : undefined, onAssignWorkflow: onAssignWorkflow ? () => onAssignWorkflow("FILE", file.id, file.name) : undefined,
                  }}
                />
              </td>
            </tr>
