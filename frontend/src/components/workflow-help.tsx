...
export function WorkflowHelp({ compact = false, title, description, tips, helpKey }: { compact?: boolean; title?: string; description?: string; tips?: string[]; helpKey?: WorkflowHelpKey }) {
  ...
  return <>
    <button type="button" onClick={() => setOpen(true)} className="workflow-help-button inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-slate-400 transition hover:bg-[#EEF4FF] hover:text-[#1B66EA] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1B66EA]/30" aria-label={ar ? "مساعدة" : "Help"}>
      <span className="flex h-5 w-5 items-center justify-center text-[11px] font-semibold leading-none">?</span>
    </button>
