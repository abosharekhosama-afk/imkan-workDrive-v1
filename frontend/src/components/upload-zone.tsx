...
export function UploadZone({ folderId, onUploaded }: { folderId: string | null; onUploaded: () => void }) {
  ...
  // Auto-dismiss (P0 UX): once every upload in the queue has settled, close the
  // success toast and drop completed rows after a short 3000ms grace period so
  // the popup clears by itself after a successful upload. Failed rows persist
  // for retry.
  useEffect(() => {
    if (items.length === 0) return;
    const hasCompleted = items.some((item) => item.status === "completed");
    const pending = items.some((item) => item.status === "queued" || item.status === "processing" || item.status === "uploading");
    if (!hasCompleted || pending) return;
