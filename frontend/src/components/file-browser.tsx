...
import { bulkTrashFolders, createFolder, deleteFolder, getFolder, listRootContents, renameFolder } from "../lib/api/folders";
import { bulkTrashFiles, renameFile, requestDownload, trashFile } from "../lib/api/files";
...
export function FileBrowser({
  ...
  async function onPreview(type: "FILE" | "FOLDER", id: string, name: string, mimeType?: string, size?: number) {
    ...
    try {
      ...
      // Store preview URL in a way that FilePreview can access it
      // We'll use a custom event or store it in state
      window.dispatchEvent(new CustomEvent("preview-url-ready", { detail: { id, url: previewUrl.download_url } }));
    } catch (cause) {
      setError(
        cause instanceof ApiError && cause.status === 401
          ? label("error.unauthenticated")
  ...
  const getPreviewableFiles = useCallback((): FileRecord[] => {
    return files.filter((f) => {
      const category = getPreviewMimeCategory(f.mimeType ?? "");
