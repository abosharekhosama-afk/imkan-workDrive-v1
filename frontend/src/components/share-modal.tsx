  ...
  return (
    <Modal title={label("share.title")} onClose={onClose}>
      <form onSubmit={onSubmit} className="text-[length:var(--imkan-font-size-ui)]">
        ...
        <label className="mb-2 flex flex-col gap-1">
          Direct recipients (user IDs)
          <input value={recipientUserIds} onChange={(event) => setRecipientUserIds(event.target.value)} className="imkan-input" placeholder="UUIDs separated by commas" />
        </label>
