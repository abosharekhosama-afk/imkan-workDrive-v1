    ...
    <div className="flex flex-col items-center gap-4 py-10 text-center" role="status">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2">
        ...
        <ZohoMenu open={open === "create"} onClose={close} labelledBy="empty-create-btn"
          onSelect={(k) => { if (k === "folder") window.dispatchEvent(new Event("workdrive:new-folder")); else if (k === "doc") window.location.href = "/files/templates?create=writer"; else if (k === "sheet") window.location.href = "/files/templates?create=sheet"; else if (k === "slide") window.location.href = "/files/templates?create=slide"; }}
        ...
        <ZohoMenu open={open === "upload"} onClose={close} labelledBy="empty-upload-btn"
          onSelect={(k) => { if (k === "files") window.dispatchEvent(new Event("workdrive:trigger-upload")); else if (k === "folderUp") window.dispatchEvent(new Event("workdrive:trigger-upload-folder")); else window.dispatchEvent(new CustomEvent("workdrive:external-apps")); }}
