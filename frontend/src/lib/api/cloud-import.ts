...
export type CloudFileListing = { files: CloudRemoteFile[]; nextPageToken: string | null; parent: string | null };
...
export const listCloudFiles = (provider: CloudProvider, connectionId?: string | null, parentId?: string | null, pageToken?: string | null) => { const q = new URLSearchParams(); if (connectionId) q.set('connectionId', connectionId); if (parentId) q.set('parentId', parentId); if (pageToken) q.set('pageToken', pageToken); return apiRequest<CloudFileListing>(`/cloud-import/${provider}/files${q.toString() ? `?${q}` : ''}`); };
