...
export type ConnectionGovernance = { id:string; name:string; connectionType:string; visibility:string; status:string; provider:string; authType:string; ownerId:string; workflowReferences:number; functionReferences:number; activeWorkflowIds:string[]; providerGovernance?: { enabled:boolean; configuredScopes:string[]; selectedScopes:string[]; requestedScopes:string[]; grantedScopes:string[]; missingScopes:string[]; configurationOnlyScopes:string[]; lastTestedAt?:string|null; lastTestOk?:boolean|null; lastTestMessage?:string|null } | null; usage:{ failures:number }; issues:Array<{code:string;severity:'BLOCKING'|'WARNING';message:string}>; readyForWorkflow:boolean; checkedAt:string };
