...
export type Connection = { canManage: boolean; id: string; orgId: string; ownerId: string; name: string; linkName?: string; connectionType?: 'USER'|'SYSTEM'|'ADMIN'; provider: string; authType: string; visibility: 'PRIVATE' | 'ORGANIZATION'; status: 'PENDING_AUTH' | 'ACTIVE' | 'REAUTH_REQUIRED' | 'DISABLED' | 'ERROR'; baseUrl?: string | null; metadata?: Record<string, unknown> | null; authorizedAccount?: {id?:string;email?:string;name?:string;username?:string} | null; expiresAt?: string | null; scope?: string | null; lastTestedAt?: string | null; lastUsedAt?: string | null; errorCode?: string | null; errorMessage?: string | null; createdAt: string; updatedAt: string };
...
export function browseConnectionResources(id: string, parent?: string) { const q = parent ? `?parent=${encodeURIComponent(parent)}` : ""; return apiRequest<{ connectionId: string; provider: string; parent: string | null; folders: ConnectionResource[]; files: ConnectionResource[] }>(`/connections/${id}/resources${q}`); }
